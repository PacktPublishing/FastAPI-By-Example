...
def get_category(db: Session, id: int):
    ...
    if not db_category:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Category not found!')
    return db_category
...
def get_categories(db: Session, skip: int = 0, limit: int = 100):
    ...
    if not db_categories:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='No categories were found!')
...
def update_category(id: int, db: Session, category: schemas.UpdateCategory):
    ...
    if not db_category:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Category does not exist!')
...
def get_flashcard(db: Session, id: int):
    db_flashcard = db.query(models.Flashcard).filter(
        models.Flashcard.id == id).first()
    if not db_flashcard:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Flashcard not found!')
    return db_flashcard
...
def get_flashcards(db: Session, skip: int = 0, limit: int = 100):
    ...
    if not db_flashcards:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Flashcards not found!')
    return db_flashcards


def create_flashcard(db: Session, flashcard: schemas.CreateFlashcard):
    db_flashcard = models.Flashcard(
        question=flashcard.question, answer=flashcard.answer, category_id=flashcard.category_id)
    db.add(db_flashcard)
    db.commit()
