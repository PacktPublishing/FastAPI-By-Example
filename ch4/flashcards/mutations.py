    ...
    async def mutate(root, info, name):
        try:
            ...
            category = crud.create_category(
                db=database.SessionLocal(), category=category_schema)
    ...
    async def mutate(root, info, id, name):
        try:
            ...
            category = crud.update_category(
                db=database.SessionLocal(), id=id, category=category_schema)
...
class CreateFlashcard(graphene.Mutation):
    ...
    async def mutate(root, info, question, answer, category_id):
        try:
            flashcard_schema = schemas.CreateFlashcard(
                question=question, answer=answer, category_id=category_id)
            flashcard = crud.create_flashcard(
                db=database.SessionLocal(), flashcard=flashcard_schema)
...
class UpdateFlashcard(graphene.Mutation):
    ...
    async def mutate(root, info, id, question=None, answer=None, category_id=None):
        try:
            flashcard_schema = schemas.UpdateFlashcard(
                question=question, answer=answer, category_id=category_id)
            flashcard = crud.update_flashcard(
                db=database.SessionLocal(), id=id, flashcard=flashcard_schema)
...
class DeleteFlashcard(graphene.Mutation):
    ...
    async def mutate(root, info, id):
        try:
            flashcard = crud.delete_flashcard(
                db=database.SessionLocal(), id=id)
